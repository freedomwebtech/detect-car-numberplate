...
client = Groq(api_key='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')

# Configure Google Generative AI with your API key (for embeddings)
os.environ["GOOGLE_API_KEY"] = "AIzaSyCLvEJ16I86mahwMda0vZQVdP27aJHkNao"  # Replace with your actual API key

# Function to extract text from an Excel file
def get_excel_content(file):
    try:
        return pd.read_excel(file)  # Read the Excel file into a DataFrame
...
if uploaded_files:
    ...
    selected_file_name = st.sidebar.selectbox("Select an Excel file", file_names)

    # Find the selected file
    selected_file = next(file for file in uploaded_files if file.name == selected_file_name)

    # Load Excel content from the selected file
    df = get_excel_content(selected_file)
    if not df.empty:
        st.write(f"Preview of selected Excel file: {selected_file_name}")
        st.dataframe(df)
    ...
    st.write("Please upload at least one Excel file.")
